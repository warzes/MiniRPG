...
{
	{{ -0.1f,  0.1f, 2.0f}, {0.0f, 0.0f}}, // top left
	{{  0.1f,  0.1f, 2.0f}, {1.0f, 0.0f}}, // top right
	{{  0.1f, -0.1f, 2.0f}, {1.0f, 1.0f}}, // bottom right
	{{ -0.1f, -0.1f, 2.0f}, {0.0f, 1.0f}}, // bottom left
};
...
void main()
{
	gl_Position = ProjectionMatrix * WorldMatrix * vec4(aPosition, 1.0);
	TexCoord    = aTexCoord;
}
)";
...
void main()
{
	FragmentColor = texture(DiffuseTexture, TexCoord);
		...
		m_uniformQuadProjectionMatrix = render.GetUniform(m_shader, "ProjectionMatrix");
		m_uniformQuadWorldMatrix = render.GetUniform(m_shader, "WorldMatrix");

	}

	m_geom = render.CreateGeometryBuffer(BufferUsage::StaticDraw,
		static_cast<unsigned>(Countof(vertQuad)), static_cast<unsigned>(sizeof(Vertex)), vertQuad,
		static_cast<unsigned>(Countof(indicesQuad)), IndexFormat::UInt32, indicesQuad,
		m_shader);
...
void main()
{
	...
	while (1)
	{
...

		{
			...
			{
				glm::mat4 world = glm::translate(glm::mat4(1.0f), {glm::vec3(-0.95f, -0.7f, 0.0f)});

				render.Bind(m_texture, 0);
				render.Bind(m_shader);
				render.SetUniform(m_uniformQuadProjectionMatrix, m_perspective);
				render.SetUniform(m_uniformQuadWorldMatrix, world);
