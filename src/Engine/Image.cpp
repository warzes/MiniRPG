...
bool Image::LoadFromMemory(const uint8_t* data, size_t dataSize)
{
	...
	int nrChannels = 0;
	m_pixelData = stbi_load_from_memory(data, dataSize, &m_width, &m_height, &nrChannels, desired—hannels);
...
bool Image::LoadFromFile(const std::string& fileName, bool verticallyFlip)
{
	...
	const int desired—hannels = STBI_default;
	int nrChannels = 0;
	m_pixelData = stbi_load(fileName.c_str(), &m_width, &m_height, &nrChannels, desired—hannels);
	m_imageFormat = convertSTBToEngine(nrChannels);
